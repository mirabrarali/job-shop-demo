...
export default function CallForm() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setError(""); setLoading(true); const result = await fetch("/api/calls/start", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone }) }); const data = await result.json(); setLoading(false); if (!result.ok) { setError(data.error || "Unable to start the call."); return; } router.push(`/call/${data.callId}`); }
